...
def create_edge_index_and_features(stations_included_file, stations_data_file, graph_file=None, compute_edge_features=True):
    ...
    print(f'Finish creating edge index and features, num nodes: {num_nodes}, num edges: {len(start_indices)}')
...
class TrafficVolumeAutoRegressiveGraphDataSet(TrafficVolumeDataSet):
    """
        Modified dataset for use with PyTorch Geometric GNN.
    """
    # Define a function to get holiday information for Norway
    def __init__(self, datafile, edge_index, edge_attr, holidays, lookback_steps):
        super().__init__(datafile)
        self.lookback_steps = lookback_steps
        self.holiday_dict = {}
        self.holiday_records = []
        if holidays:
            with open(holidays, 'rb') as file:
                self.holiday_dict = pickle.load(file)
            self.holiday_records = sorted(set(self.holiday_dict.values()))
        self.edge_index, self.edge_attr = edge_index, edge_attr

    def get_holidays(self, data):
        holiday_vec = [0] * len(self.holiday_records)
        timestamp = getattr(data, "name")
        date_str = timestamp.strftime('%Y-%m-%d')
        if date_str in self.holiday_dict:
            holiday = self.holiday_dict[date_str]
            idx = self.holiday_records.index(holiday)
            holiday_vec[idx] = 1
        return holiday_vec
    ...
    def __getitem__(self, index):
        ...
        data_now = self.df.iloc[index].replace(np.nan, -1)
        data_next = self.df.iloc[index + 1].replace(np.nan, -1)
        month_weekday_hour = self.convert_time(data_now)
        holiday_vec = self.get_holidays(data_now)
        time_info = month_weekday_hour + holiday_vec
        datetime = torch.Tensor(time_info).reshape(1, len(time_info))

        volume_now = data_now.to_numpy(dtype=np.float32)
        volume_now = volume_now.reshape(-1, 1)  #
        volumes = np.repeat(volume_now, self.lookback_steps, axis=1)
        for i in range(1, self.lookback_steps):
            if index-i >= 0:
                data_past = self.df.iloc[index-i].replace(np.nan, -1)
                volumes[:, i] = data_past.to_numpy(dtype=np.float32)
...
def TrafficVolumeAutoRegressiveGNNDataLoader(datafile, edge_index, edge_attr, batch_size=32, num_workers=4, shuffle=False, drop_last=False, holidays=None, lookback_steps=1):
    dataset = TrafficVolumeAutoRegressiveGraphDataSet(datafile, edge_index, edge_attr, holidays, lookback_steps)
    dataloader = torch_geometric.loader.DataLoader(dataset, batch_size=batch_size, shuffle=shuffle, num_workers=num_workers, drop_last=drop_last, pin_memory=True)
    return dataloader